      ...
      <div className="container px-4 mx-auto">
        <div className="max-w-4xl mx-auto text-center space-y-8 mb-20">
          <div className="space-y-4">
            ...
            <h2 className="text-5xl font-serif font-bold">Build Your Career With Us</h2>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-24">
          {jobs.map((job, i) => (
            <div
              key={i}
              className="bg-white p-8 rounded-3xl shadow-sm border border-border/50 hover:shadow-xl transition-all group"
            >
              <div className="flex justify-between items-start mb-6">
                <div className="w-14 h-14 rounded-2xl bg-accent/10 flex items-center justify-center group-hover:bg-accent group-hover:text-white transition-colors">
                  <Briefcase className="h-7 w-7" />
                </div>
                <Badge
                  variant="secondary"
                  className="bg-accent/10 text-accent border-none font-bold uppercase tracking-wider px-4 py-1"
                >
                  {job.type}
                </Badge>
              </div>
              ...
              <Button className="w-full bg-background hover:bg-accent hover:text-white text-foreground border border-border group-hover:border-accent rounded-full py-6 font-bold transition-all">
                Apply Now
              </Button>
            </div>
          ))}
        ...
        {/* Application Section */}
        <div className="bg-white p-8 md:p-16 rounded-[40px] shadow-sm border border-border/50">
          <div className="text-center space-y-4 mb-12">
            <h3 className="text-3xl font-serif font-bold">Can't find the right role?</h3>
            <p className="text-muted-foreground">Submit your CV and we'll keep you in mind for future openings.</p>
          </div>
          <form className="max-w-2xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Input placeholder="LinkedIn Profile (Optional)" className="rounded-xl h-12 md:col-span-2" />
            <div className="md:col-span-2 border-2 border-dashed border-border rounded-2xl p-8 text-center space-y-2 hover:border-accent transition-colors cursor-pointer">
              <p className="font-bold">Upload Your CV</p>
              <p className="text-sm text-muted-foreground">PDF, DOC, DOCX up to 5MB</p>
            </div>
            <Button className="md:col-span-2 bg-accent hover:bg-accent/90 text-accent-foreground py-8 rounded-full text-lg font-bold">
              Submit Application
            </Button>
          </form>
        </div>
